...
const Qurl = process.env.REACT_APP_QUESTION_URL
...
function BrowseQuestions() {
  ...
  useEffect(() => {
    const service = keywordId ? `/by/keywords?id=${keywordId}` : ``

    axios.get(`${Qurl}/count${service}`)
  ...
  useEffect(() => {
    ...
    const service = keywordId ? `questions/byKeywords?id=${keywordId}` : `questions/page`
    const sep = keywordId ? '&' : '?'
    const config = { headers: { 'Authorization': `Bearer ${token}` } }

    axios.get(`${Burl}/${service}${sep}pagesize=${pageSize}&pagenr=${currentPage}`, config)
