...
const Burl = process.env.REACT_APP_BROWSING_URL
const Aurl = process.env.REACT_APP_ANSWER_URL
...
function Question() {
  ...
  useEffect(() => {
    ...
    axios.get(`${Burl}/question/${id}`, config)
      ...
      <div  className='w-100' style={{ maxWidth: '950px' }}>
        ...
        <Card.Body className='question'>
          ...
          <p className='date-user text-muted mb-0 px-0'>
            asked on {question.createdAt && formatDate(question.createdAt)} by {question.user?.displayName ? question.user.displayName : '[deleted]'}
          </p>
        </Card.Body>
            ...
            question.answers.map(answer => (
                <Card className='mb-2' key={answer.id}>
                  <ListGroup.Item className='border-0'>
                    ...
                    <p className='date-user text-muted mb-0 px-0'>
                      answered on {answer.createdAt && formatDate(answer.createdAt)} by {answer.user ? answer.user.displayName : '[deleted]'}
                    </p>
                  </ListGroup.Item>
                </Card>
            ))}
